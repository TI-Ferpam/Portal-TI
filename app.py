...
if st.session_state.tela == "busca":
    ...
    if not st.session_state.autenticado_admin:
        st.write("Digite o **Número do Chamado** ou o **Seu Nome** para pesquisar seu atendimento.")
        ...
        if btn_pesquisar or input_ticket.strip() or input_nome.strip():
            res = df.copy()
            ...
            if not input_ticket.strip() and not input_nome.strip():
                st.info("💡 Digite um número de ticket ou seu nome acima para iniciar a busca.")
            elif res.empty:
                st.warning("Nenhum chamado encontrado com esses dados.")
            else:
