...
<style>
    ...
    /* CORREÇÃO DAS CORES DOS CARDS E ABAS (PREVINE MISTURA DE BRANCO COM BRANCO) */
    ...
    /* Estilização clara para botões de aba */
...
if st.session_state.tela == "dashboard":
    ...
    # ============================================================
    # TAB 1: OPERAÇÃO & VOLUMETRIA (COM FILTRO DE ANO E MÊS)
    # ============================================================
    with tab_op:
    ...
    # ============================================================
    # TAB 2: SLAS & MÉDIAS DE TEMPO (SEM GRÁFICOS INTERATIVOS)
    # ============================================================
    with tab_sla:
        ...
        with c1:
            st.metric("Chamados SLA Válidos", len(df_sla))
        with c2:
            med_assumir = df_sla["min_ate_tecnico"].mean() if not df_sla.empty else 0
            st.metric("Tempo Médio até Assumir", formatar_tempo(med_assumir))
        with c3:
            med_exec = df_sla["min_resolucao"].mean() if not df_sla.empty else 0
            st.metric("Tempo Médio de Execução", formatar_tempo(med_exec))
        with c4:
            med_total = df_sla["min_total"].mean() if not df_sla.empty else 0
            st.metric("Tempo Total Médio", formatar_tempo(med_total))
        ...
        if not df_sla.empty:
            df_tec_sla = df_sla.groupby("tecnico").agg(
                ...
                med_execucao=("min_resolucao", "mean"),
                med_total=("min_total", "mean")
            ).reset_index()

            df_tec_sla["Tempo Médio até Assumir"] = df_tec_sla["med_assumir"].apply(formatar_tempo)
            df_tec_sla["Tempo Médio de Execução"] = df_tec_sla["med_execucao"].apply(formatar_tempo)
            df_tec_sla["Tempo Total Médio"] = df_tec_sla["med_total"].apply(formatar_tempo)

            df_tec_sla_display = df_tec_sla[["tecnico", "qtd_chamados", "Tempo Médio até Assumir", "Tempo Médio de Execução", "Tempo Total Médio"]]
            df_tec_sla_display.columns = ["Técnico", "Qtd Chamados SLA", "Tempo Médio até Assumir", "Tempo Médio de Execução", "Tempo Total Médio"]
            st.dataframe(df_tec_sla_display, use_container_width=True, hide_index=True)
        else:
            st.warning("Nenhum chamado com 3 datas válidas registrado para exibir o comparativo por técnico.")
